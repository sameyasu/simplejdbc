 ...
 * @param <E> エンティティクラス
 */
public class SelectJdbcImplMysql<E> extends AbstractOrMappingJdbc<E> implements SelectJdbc<E> {

	/**
	 * ロガー
	 */
	private Log log = LogFactory.getLog(SelectJdbcImplMysql.class);
	 ...
	 */
	public SelectJdbcImplMysql(Connection connection, Class<E> clazz) {
	...
	public void setSelectForUpdate(SelectForUpdate forUpdate){
		if(forUpdate != null) {
			if(forUpdate.getWaitSeconds() == SelectForUpdate.WAITSECONDS_NOWAIT) {
				//mysqlはNOWAITに非対応
				log.warn("MysqlはSELECT FOR UPDATE NOWAITに対応していません。");
			}
			else if(forUpdate.getWaitSeconds() != SelectForUpdate.WAITSECONDS_WAIT_UNLIMITED) {
				//mysqlはWAITに非対応
				log.warn("MysqlはSELECT FOR UPDATE WAITに対応していません。");
			}
		}
