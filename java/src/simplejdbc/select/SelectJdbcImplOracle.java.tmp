 ...
 * @param <E> エンティティクラス
 */
public class SelectJdbcImplOracle<E> extends AbstractOrMappingJdbc<E> implements SelectJdbc<E> {

	/**
	 * ロガー
	 */
	private Log log = LogFactory.getLog(SelectJdbcImplOracle.class);
	 ...
	 */
	public SelectJdbcImplOracle(Connection connection, Class<E> clazz) {
	...
	protected String getForUpdateSqlString() {
		if(this.selectForUpdate != null){
			if(this.selectForUpdate.getWaitSeconds() == SelectForUpdate.WAITSECONDS_NOWAIT) {
				...
				return " FOR UPDATE WAIT "+this.selectForUpdate.getWaitSeconds();
			}
		}
		else {
			return "";
