 ...
 * @param <E> エンティティクラス
 */
public class InsertJdbcImplOracle<E> extends AbstractManageVersionJdbc<E> implements InsertJdbc<E> {

	/**
	 * ロガー
	 */
	private Log log = LogFactory.getLog(InsertJdbcImplOracle.class);
	 ...
	 */
	public InsertJdbcImplOracle(Connection connection, Class<E> entityClass) {
	...
	public int insert(E entityInstance) throws SQLException {
		...
		for(String fieldName : this.fieldMap.keySet()){
			...
			if(this.isIgnoreVersion() == false && jdbcColumn.isVersionColumn()){
				String newVersion = this.getNewVersionOracle();
				try {
		...
		for(String fieldName : this.fieldMap.keySet()){
			...
			if(column.isIncrementValue() && value == null){
				...
				if(incrementValue.sql().equals("")){
					sql.append(incrementValue.sequence()).append(".NEXTVAL");
				}
				else {
